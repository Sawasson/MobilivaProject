...
namespace Mobiliva.Common.EventBus
{
    public class EventBusRabbitMQ : IEventBus, IDisposable
    {
        ...
        public EventBusRabbitMQ(IEventLogService eventLogService, IRabbitMQPersistentConnection persistentConnection, ILifetimeScope autofac, IEventBusSubscriptionsManager subsManager, string queueName = null, int retryCount = 5)
        {
        ...
        public void Publish(IFEvent @event)
        {
           ...
           // eventLogService.SaveEventAsync(@event, _queueName, EventStateEnum.Publishing);
            ...
            var policy = RetryPolicy.Handle<BrokerUnreachableException>()
                ...
                {
                    eventLogService.SaveEventAsync(@event, _queueName, EventStateEnum.PublishedFailed);
            ...
            using (var channel = _persistentConnection.CreateModel())
            {
                ...
                policy.Execute(() =>
                {
                    ...
                    //eventLogService.SaveEventAsync(@event, _queueName, EventStateEnum.Published);
                });
            }
                ...
                using (var scope = _autofac.BeginLifetimeScope(AUTOFAC_SCOPE_NAME))
                {
                    ...
                    foreach (var subscription in subscriptions)
                    {
                        ...
                        else
                        {
                            ...
                            //await eventLogService.SaveEventAsync((IFEvent)@event, _queueName, EventStateEnum.Processing);
                            var handler = scope.ResolveOptional(subscription.HandlerType);
                            var concreteType = typeof(IIFEventHandler<>).MakeGenericType(eventType);
                            await (Task)concreteType.GetMethod("Handle").Invoke(handler, new object[] { @event });
                            //await eventLogService.SaveEventAsync((IFEvent)@event, _queueName, EventStateEnum.Processed);
                        }
                    }
                }
            }
